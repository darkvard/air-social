...
|------------|--------------|-------|
| Show version | `make migrate-version` | Prints current migration version. |
| Create migration | `make migrate-create name=add_users` | Generates `xxx_add_users.up.sql` & `.down.sql`. |
| Apply migrations (all / n) | `make migrate-up n=1` | No `n` → apply **all**; with `n` → apply **next n**. |
| Rollback migrations (all / n) | `make migrate-down n=1` | No `n` → rollback **all**; with `n` → rollback **last n**. |
